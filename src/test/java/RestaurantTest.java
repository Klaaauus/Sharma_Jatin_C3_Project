...
class RestaurantTest {
    ...
    public void is_restaurant_open_should_return_true_if_time_is_between_opening_and_closing_time(){
        Mockito.doReturn(LocalTime.parse("20:00:00")).when(restaurant).getCurrentTime();
        assertEquals(true,restaurant.isRestaurantOpen());
    }

    @Test
    public void is_restaurant_open_should_return_false_if_time_is_outside_opening_and_closing_time(){
        Mockito.doReturn(LocalTime.parse("23:30:00")).when(restaurant).getCurrentTime();
    ...
    @Test
    public void calculate_total_order_should_be_equal_to_sum_of_selected_items(){
        List<String> selectedItems = new ArrayList<>();
        selectedItems.add("Sweet corn soup");
        selectedItems.add("Vegetable lasagne");
        assertEquals(388,restaurant.calculateTotalOrder(selectedItems));
    }
    //<<<<<<<<<<<<<<<<<<<<Calculate Total Order>>>>>>>>>>>>>>>
}